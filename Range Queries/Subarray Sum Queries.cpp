...
void build(int node , int start , int end) {
    if(start==end) {
        ...
        segtree[node].seg = max(0LL,a[end] ) ; 
        segtree[node].pre = max(0LL,a[end]) ; 
        segtree[node].suf = max(0LL,a[end]) ; 
        return ;
    }
    int mid = (start+end) / 2 ;
    int left = node*2 ;
    ...
    segtree[node].pre = max(segtree[left].pre , segtree[left].sum + segtree[right].pre ) ; 
    segtree[node].suf = max(segtree[right].suf, segtree[right].sum + segtree[left].suf ) ;  
    segtree[node].seg = max(segtree[node].sum , segtree[left].suf + segtree[right].pre) ;
    segtree[node].seg = max( { segtree[left].seg , segtree[right].seg , segtree[node].seg } ) ; 
}
...
void solution(int cs) {
    ...
    for(ll i=1;i<=m;i++) {
        ...
        cout << max({segtree[1].seg,segtree[1].pre,segtree[1].suf,segtree[1].sum}) el ; 
    }
}             
