...
void update(int node , int start , int end , int l , int r , ll x ) {
    ...
    if(l<=start and end<=r){
        segtree[node].sum += ((end - start + 1) * x);
        segtree[node].prop += x;
        return ;
    }
    int mid = (start+end) / 2 ;
    ...
    segtree[node].sum = segtree[left].sum + segtree[right].sum + (end - start + 1) * segtree[node].prop;
}
 
ll query(int node, int start, int end , int l,  int r, ll carry = 0)
{
    if (l > end || r < start) {
        return 0;
    }
    if (start >= l and end <= r) {
        return segtree[node].sum + carry * (end - start + 1) ;
    } 
    int mid = (start+end) / 2 ;
    int left = node*2 ;
    int right = node*2 + 1 ;
    ll p1 = query(left, start , mid, l , r , carry + segtree[node].prop); 
    ll p2 = query(right, mid + 1, end , l , r , carry + segtree[node].prop);
