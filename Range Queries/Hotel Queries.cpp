...
void update(int node , int start , int end , int l , int r , ll x ) {   // reduce the index value 
...
ll query(int node, int start, int end , int x ) {      // return the index that we can take
    if(start==end and segtree[node].sum>=x) {
        return end ; 
    }
    else if(segtree[node].sum<x) {
        return 0 ; 
    }
    int mid = (start+end)/2 ; 
    int left = node * 2 ; 
    ...
    if(segtree[left].sum>=x) {
        ...
        return query(right,mid+1,end,x) ;  
    }
}
...
void solution(int cs) {
    ...
    for(ll i=1;i<=m;i++) {
        cin >> tour[i] ; 
        int index = query(1,1,n,tour[i]) ; 
        cout << index << " " ;  
        update(1,1,n,index,index,-tour[i]) ; 
    }
}             
